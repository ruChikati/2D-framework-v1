...
class Entity:
    ...
    def update(self):
        collision_directions, amount_moved = self.move(
            self.game.assets.worlds.current_world.level.get_collision_mesh(
                (
                    self.rect.topleft,
                    self.rect.topright,
        ...
        if collision_directions["right"] or collision_directions["left"]:
            self.vel[0] = 0
        self.game.assets.camera.render(
            self.img,
            (
                self.x + self.w // 2 - self.img.get_width(),
                self.y + self.h // 2 - self.img.get_height(),
            ),
        )
...
class Player(Entity):
    def __init__(self, x, y, w, h, img, game, gravity=0):
        ...
        self.anims = {
            anim.split(self.name)[-1]: self.game.assets.anims.anims[anim]
            for anim in self.game.assets.anims.anims
            if self.name in anim
        }
    ...
    def update(self):
            ...
            if event.type == input.KEYHOLD:
                match event.key:
                    case input.SPACE:
                        self.vel[1] -= (
                            self.speed
                            * math.sin(math.radians(self.fwd))
                            * self.accel_speed
                        )
                        self.vel[0] += (
                            self.speed
                            * math.cos(math.radians(self.fwd))
                            * self.accel_speed
                        )
                        self.game.particles.append(
                            particle.ParticleBurst(
                                ...
                                80,
                                (
                                    -self.speed * math.cos(math.radians(self.fwd)),
                                    self.speed * math.sin(math.radians(self.fwd)),
                                ),
                                self.game,
                                type="y",
                                shape="rect",
                    ...
                    case input.A:
                        ...
                        self.game.particles.append(
                            particle.ParticleBurst(
                                ...
                                80,
                                (
                                    self.rot_speed * -math.cos(math.radians(-self.fwd)),
                                    self.rot_speed * math.sin(math.radians(-self.fwd)),
                                ),
                                self.game,
                                type="-",
                                shape="rect",
        ...
        collision_directions, amount_moved = self.move(
            self.game.assets.worlds.get_active_world().level.collision_mesh,
            self.game.dt,
        )
        ...
        self.anims[self.action].img = pygame.transform.rotate(
            self.anims[self.action].img, self.fwd
        )
